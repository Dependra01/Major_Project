...
const corsOptions = {
    origin: ['https://stock-market-prediction-theta.vercel.app'], //'http://localhost:3000',
...
app.post("/getStockData", async (req, res) => {
  try {
    ...
    // Convert the startDate and endDate to timestamps
    const period1 = new Date(startDate).getTime() / 1000; // Convert to seconds
    const period2 = new Date(endDate).getTime() / 1000;   // Convert to seconds

    // Use chart() instead of historical()
    const df = await yfinance.chart(stockSymbol, {
      period1, // Timestamp for startDate
      period2  // Timestamp for endDate
    });
